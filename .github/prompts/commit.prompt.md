...
[ ] 2. 執行 `git status` 
  [ ] 2-1. 檢查是否存在 `.github` 資料夾，沒有 => 跳過目前這條任務，有 => 再檢查 `.github` 資料夾內是否有 git 版本控制，有 => 跳過目前這條任務，沒有 => 加入暫存
  [ ] 2-2. 檢查哪些檔案已暫存，如果沒有檔案暫存，會自動使用 `git add` 新增所有修改和新增的檔案
...
## 讀取 Windows 注意事項文件(使用 Read 工具)

用下列方法讀取檔案內容
- #file:../instructions/windows-command-notice.md
- [windows-command-notice.md](../instructions/windows-command-notice.md)
- @windows-command-notice.md
